...
public class AuthServiceImpl implements AuthService {
    ...
    public String login(String username, String password) {
        ...
        logger.info("加密之后的密码: "+requestMd5);
        String dbMd5 = user.getPassword();
        logger.info("数据库存储密码：" +dbMd5);

        if (dbMd5 == null || !dbMd5.equalsIgnoreCase(requestMd5)) {
            logger.info("加密之后的密码requestMd5: "+requestMd5+"    "+dbMd5);
            throw new AuthException(AuthExceptionEnum.USERNAME_PWD_ERROR);
    ...
    public String login(String username) {
        ...
        User user = userMapper.getByAccount(username);
        logger.info("登陆的的用户: "+user.getName());
        System.out.println("登陆的的用户: "+user.getName());

        // 账号不存在
        if (null == user) {
            throw new AuthException(AuthExceptionEnum.USERNAME_PWD_ERROR);
        }
        logger.info("AAA: "+user.getName());
    ...
    public void addLoginCookie(String token) {
        ...
        Cookie roadlakecookietoken = new Cookie("road-lake-cookie-token", token);
        roadlakecookietoken.setMaxAge(getJwtSecretExpireSec().intValue());
        roadlakecookietoken.setHttpOnly(false);
        roadlakecookietoken.setPath("/");
        response = HttpContext.getResponse();
        response.addCookie(roadlakecookietoken);
    }
